...
import { Building, Home, Briefcase, Map, Hotel, ArrowRight } from "lucide-react";
...
export default async function CategorySection() {
  ...
  const colors = {
    apartment: "from-blue-500/10 to-indigo-500/10 text-blue-600 dark:text-blue-400 border-blue-100 dark:border-blue-900/30",
    villa: "from-emerald-500/10 to-teal-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-900/30",
    commercial: "from-amber-500/10 to-orange-500/10 text-amber-600 dark:text-amber-400 border-amber-100 dark:border-amber-900/30",
    land: "from-sky-500/10 to-cyan-500/10 text-sky-600 dark:text-sky-400 border-sky-100 dark:border-sky-900/30",
    penthouse: "from-violet-500/10 to-purple-500/10 text-violet-600 dark:text-violet-400 border-violet-100 dark:border-violet-900/30",
  };
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-2 gap-5 md:grid-cols-3 lg:grid-cols-5">
          {categories.map((category) => {
            ...
            const theme = colors[slug.toLowerCase()] || "from-neutral-500/10 to-slate-500/10 text-neutral-600 dark:text-neutral-400 border-neutral-100 dark:border-neutral-800";
            ...
            return (
              ...
              >
                {/* Icon Container */}
                <div className={`flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br border ${theme} transition-transform duration-300 group-hover:scale-105 shadow-xs`}>
                  {getIcon(category.slug)}
                </div>

                {/* Name */}
                <h3 className="mt-5 text-sm font-extrabold text-neutral-950 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                  {category.name}
                </h3>
