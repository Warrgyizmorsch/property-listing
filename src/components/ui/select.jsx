    ...
    <SelectPrimitive.Portal>
      <SelectPrimitive.Positioner className="isolate z-50 outline-none" sideOffset={4}>
        <SelectPrimitive.Popup
          className={cn(
            "z-50 max-h-(--available-height) w-(--anchor-width) min-w-[8rem] overflow-x-hidden overflow-y-auto rounded-lg border border-neutral-200/85 bg-white p-1 text-neutral-800 shadow-md outline-none dark:border-zinc-800 dark:bg-zinc-900 dark:text-neutral-100 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
            className
          )}
          {...props}
        >
...
function SelectItem({ className, children, ...props }) {
  return (
    <SelectPrimitive.Item
      className={cn(
        "relative flex w-full cursor-pointer select-none items-center rounded-md py-1.5 pl-8 pr-2 text-sm font-semibold text-neutral-700 outline-none hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-zinc-800 data-disabled:pointer-events-none data-disabled:opacity-50",
        className
      )}
      {...props}
    >
      <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
        <SelectPrimitive.ItemIndicator>
          <CheckIcon className="h-4 w-4 text-neutral-900 dark:text-white" />
        </SelectPrimitive.ItemIndicator>
      </span>
